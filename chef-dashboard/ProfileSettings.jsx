...
// Interface ProfileSettingsProps is removed.
export function ProfileSettings({ chefData }) {
  // Type annotation removed from useState
  const [profileData, setProfileData] = useState(chefData);
  const [hasChanges, setHasChanges] = useState(false);
  // Type annotation removed from useRef
  ...
  // Type annotations removed from updateProfileData arguments
  const updateProfileData = (field, value) => {
    setProfileData((prev) => ({ ...prev, [field]: value }));
    setHasChanges(true);
  };

  // Type annotations removed from handleSpecialtyChange arguments
  const handleSpecialtyChange = (specialty, checked) => {
    ...
    if (checked) {
      ...
      updateProfileData('specialties', currentSpecialties.filter((s) => s !== specialty));
    }
  };

  // Type annotations removed from handleCertificationChange arguments
  const handleCertificationChange = (certification, checked) => {
    ...
    if (checked) {
      ...
      // Type assertion removed
      updateProfileData('certifications', currentCerts.filter((c) => c !== certification));
    }
  };

  // Type annotation removed from handleMainImageUpload argument
  const handleMainImageUpload = (event) => {
    const file = event.target.files?.[0];
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => {
        // Type assertion `as string` removed
        updateProfileData('mainImagePreview', reader.result);
      };
          ...
          {certificationOptions.map((certification) => (
            <div key={certification} className="flex items-center space-x-2 p-3 rounded-2xl border border-border hover:bg-accent/50 transition-colors">
              <Checkbox
                ...
                // Type assertion removed
                onCheckedChange={(checked) => handleCertificationChange(certification, checked)}
              />
