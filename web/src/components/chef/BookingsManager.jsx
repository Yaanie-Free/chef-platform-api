      ...
      <div className="space-y-4">
          ...
          <Card key={booking.id} className="p-6 rounded-3xl border-border/40">
            <div className="flex flex-col lg:flex-row gap-6">
              <div className="flex items-start gap-4">
                <ImageWithFallback src={booking.clientImage} alt={booking.clientName} className="w-16 h-16 rounded-2xl object-cover" />
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <h3 className="text-lg">{booking.clientName}</h3>
                    <Badge className={`rounded-xl ${getStatusColor(booking.status)}`}>{booking.status}</Badge>
                  </div>
              ...
              <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-3">
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span>{new Date(booking.date).toLocaleDateString('en-GB', { weekday: 'short', day: 'numeric', month: 'short', year: 'numeric' })}</span>
                  </div>
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <div className="text-sm">
                      ...
                      <div className="flex flex-wrap gap-1 mt-1">
                        {booking.dietaryRequirements.map((req) => (
                          <Badge key={req} variant="secondary" className="rounded-xl text-xs">{req}</Badge>
                        ))}
                      </div>
                    </div>
                  )}
              ...
              {booking.status === 'pending' && (
                <div className="flex lg:flex-col gap-2">
                  <Button className="rounded-2xl bg-gradient-to-br from-pink-500 to-rose-500 text-white hover:from-pink-600 hover:to-rose-600">
        ...
        {filteredBookings.length === 0 && (
          <Card className="p-12 rounded-3xl border-border/40 text-center">
            ...
            <p className="text-muted-foreground">You don't have any {bookingFilter} bookings at the moment.</p>
          </Card>
        )}
      </div>
    </div>
